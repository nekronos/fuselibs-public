...
namespace Fuse.Controls.Internal
{
	block Common
	{
		...
		ClipPosition: req(LocalPosition as float2)
			Vector.Transform( LocalPosition, Matrix.Mul(Drawable.WorldTransform, Viewport.ViewProjectionTransform));
	...
	class Rectangle
	{
			...
			float2 Position = float2(0), float Smoothness = 1)
		{
			...
			Draw(viewport, dc ,drawable, Size, CornerRadius, stroke.Brush, sc,
